"""tests for proxy_guard.upstream module"""
...
def test_proxy_manager_init_list():
    """test ProxyManager initialization from a list of URLs"""
...
def test_proxy_manager_init_file(tmp_path):
    """test ProxyManager initialization from a file"""
...
def test_proxy_manager_mixed(tmp_path):
    """test ProxyManager with both list and file sources"""
    f = tmp_path / "proxies.txt"
...
def test_proxy_manager_deduplication(tmp_path):
    """test that duplicate proxies are deduplicated"""
    f = tmp_path / "proxies.txt"
...
def test_get_proxy_healthy_filtering():
    """test that get_proxy prefers healthy proxies"""
...
def test_get_proxy_empty_pool():
    """test that get_proxy returns None for an empty pool"""
    pm = ProxyManager([])
    assert pm.get_proxy() is None


def test_get_proxy_all_unhealthy_fallback():
    """test that get_proxy falls back when all proxies are unhealthy"""
    pm = ProxyManager(["http://1.1.1.1:8080", "http://2.2.2.2:8080"])
    for p in pm.proxies:
        p.is_healthy = False
        p.latency = -1.0
...
def test_get_proxy_latency_preference():
    """test that get_proxy prefers lower latency proxies"""
    pm = ProxyManager(["http://1.1.1.1:8080", "http://2.2.2.2:8080"])
    pm.proxies[0].latency = 50.0  # low latency
    pm.proxies[1].latency = 9999.0  # very high latency

    # lowusage mode should prefer low latency
    choices = set()
    for _ in range(50):
...
def test_get_proxy_high_usage_spread():
    """test that get_proxy spreads load under high usage"""
    pm = ProxyManager(["http://1.1.1.1:8080", "http://2.2.2.2:8080"])
    pm.proxies[0].latency = 50.0
...
def test_get_proxy_exclude():
    """test that get_proxy respects the exclude list"""
    pm = ProxyManager(["http://1.1.1.1:8080", "http://2.2.2.2:8080"])
    pm.proxies[0].latency = 50.0
    pm.proxies[1].latency = 50.0
...
def test_sticky_session():
    """test that sticky sessions return the same proxy"""
    pm = ProxyManager(["http://1.1.1.1:8080", "http://2.2.2.2:8080"])
    pm.proxies[0].latency = 50.0
    pm.proxies[1].latency = 50.0
...
def test_sticky_session_expires():
    """test that expired sticky sessions pick a new proxy"""
    pm = ProxyManager(["http://1.1.1.1:8080", "http://2.2.2.2:8080"])
    pm.proxies[0].latency = 50.0
    pm.proxies[1].latency = 50.0
    ...
    pm._sticky_map["test-client"] = (  # pylint: disable=protected-access
        first,
        time.time() - 1,
    )
...
def test_circuit_breaker():
    """test that the circuit breaker trips after consecutive failures"""
...
def test_hot_reload(tmp_path):
    """test that hot reload picks up new proxies from file"""
    f = tmp_path / "proxies.txt"
    f.write_text("http://1.1.1.1:8080")
    ...
    pm._reload_file()  # pylint: disable=protected-access
    assert len(pm.proxies) == 2
...
def test_hot_reload_removes_proxy(tmp_path):
    """test that hot reload removes proxies no longer in file"""
    f = tmp_path / "proxies.txt"
    f.write_text("http://1.1.1.1:8080\nhttp://2.2.2.2:8080")
